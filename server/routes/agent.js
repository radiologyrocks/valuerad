...
router.post('/agent/run', requireRole(ROLES.ADMIN, ROLES.EXECUTIVE, ROLES.SCHEDULER, ROLES.AUTH_SPECIALIST), async (req, res) => {
  ...
  // `data` carries domain context the agent's tools read (waitlist, radiologists,
  // coverage, etc.) until the EHR-backed providers are wired for each tenant.
  const services = {
    store,
    queue,
    sessionId: req.headers['x-valuerad-session'] ?? null,
    scheduling: data.scheduling ?? { findOpenSlots: async () => data.slots ?? [], bookAppointment: async (a) => a },
    fhir: null,
    ...data,
  };
